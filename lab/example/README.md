...
| Architekturtest | Konfig (depguard, import-linter) vs. Test-Framework (Konsist, ArchUnit, NetArchTest) vs. Skript (C++ `arch-check.sh`) — und in **jedem** Skelett derselbe deklarative Zweitsensor daneben ([ADR-0016](docs/plan/adr/0016-a-check-in-allen-skeletten.md)). Der Vergleich zeigt damit auch, was die Bauform einer Regel ausmacht: Konsist prüft `file.imports`, ArchUnit den Bytecode — dieselbe Umgehung, verschiedene Sichtbarkeit |
