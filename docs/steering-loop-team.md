...
|---|---|---|
| Modul `vcs`, verteilt als `doc-immutable` | `.git` plus `--range base..head` oder `--staged` | schützt den Core und meldet die **Löschung** einer immutablen Datei als `core-drift-vcs`; eine **Umbenennung mit unverändertem Inhalt** meldet nur der `--staged`-Modus, nicht der Range-Modus (gemessen, s. u.) |
...
**Für die Unveränderlichkeit des BEO-Pfads reicht `vcs` heute nicht** — und
dieser Absatz behauptete bis zur Probe das Gegenteil. Die normative Anforderung
...
nennen den Rename-Fall ausdrücklich; gemessen hängt er am **Eingabe-Modus**.
Über `--staged` meldet dieselbe Umbenennung `core-drift-vcs`
([`lab/team-sim`](../lab/team-sim/README.md) s15a), über `--range` bleibt sie
still (s15b) — und der stille ist der CI-Pfad. Erst wenn die Datei dabei
umformuliert wird, erscheint die Delete-Hälfte und mit ihr der Befund (s15c).
Der Schutz von Namespace und Slug war damit **offen** — als Werkzeug-Befund an
`vcs` (Bericht: `d--check-befund-vcs-rename.md`, dieses Verzeichnis), nicht
als Erweiterung des beantragten Vertrags. **d-check hat den Befund bestätigt
und in `v0.71.1` behoben** (Range-Diff ohne Rename-Erkennung, alle vier Fälle
nachgefahren); dieses Repo pinnt `v0.67.0`, die Messungen s15b/s16c gelten
also für den Stand vor dem Fix. Löschung und Inhaltsänderung eines
**Belegs** meldet `vcs` dagegen wie zugesagt (s16a/s16b); der Lauf gehört in
den PR beziehungsweise als `--staged`-Lauf in einen lokalen Hook.
...
und Verweis auf den korrigierenden Slice oder PR. **Die dritte Richtung fehlt
heute maschinell:** Weil der Dateiname eines Belegs *die* Slice-Kennung ist,
macht eine reine Umbenennung den Zähler nicht falsch, sondern den Beleg — die
Datei behauptet danach einen anderen Slice, und nichts meldet es (s16c,
...
Merge nicht umbenannt — **maschinell gehalten ist das heute nur halb**: Die
Löschung meldet `vcs`, die reine Umbenennung nur im `--staged`-Modus (s15a/b).
...
| Rename und Delete von Namespace oder Slug melden `core-drift-vcs` | s15a/b/c | **Werkzeug-Befund**: der reine Rename bleibt im Range-Modus still, im `--staged`-Modus nicht |
| Änderung oder Löschung eines bestehenden Belegs wird gemeldet | s16a/b/c | `core-drift-vcs` wie zugesagt — die **Umbenennung** eines Belegs bleibt still (s16c) |
...
s18). **Eine** galt als gelöst und ist es nicht (s15b): Sie braucht keine neue
Fähigkeit, sondern eine Reparatur an einer vorhandenen.
