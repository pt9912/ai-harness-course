...
| Faden | Trigger | Stand |
|---|---|---|
| **Discovery-/Kandidaten-Register** — eigener Kanal für Nicht-Slice-Register im Planning-Layout | **ein Repo zeigt den Druck NACH dem Welle-33-Fix** — also obwohl `done/` als Heimat abgeschlossener Nicht-Slice-Records sanktioniert ist | vertagt seit Welle 33, Trigger in Welle 57 neu geschnitten. *Warum:* Der alte Trigger („ein **zweites** Repo zeigt denselben Druck") zählte die Ursprungs-Beobachtung mit — sie stammt aus `m-trace` und entstand **vor** der Korrektur, die auf sie hin gemacht wurde. Vor-Fix-Druck belegt nichts mehr; die Frage ist, ob der Fix *reicht*. Beleglage: **eine** Vor-Fix-Beobachtung (`m-trace`, von hier nicht nachmessbar), **eine** Nach-Fix-Messung mit negativem Ergebnis (`ai-harness-init` nutzt `done/` wie vorgesehen, Welle 56). `d-check` stand hier fälschlich als Beleg — es war nie die Druckquelle |
