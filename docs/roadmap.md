...
|---|---|---|---|
| 2026-08-15 | Faden **`lab/example` hat keine Durchsetzungsschicht** | geschlossen — Job `example-verify` in `checks.yml` | Der Widerspruch stand seit Welle 72: Vier d-check-Module und der Vorführ-Gegenstand aus Modul 11 liefen nur lokal, während `checks.yml` im eigenen Kopf *„Ein Gate, das nur lokal läuft, ist ein Vorschlag"* trägt. Der Job ist **ein** `make`-Aufruf (`make -C lab/example verify`), weil der Lauf hermetisch ist — Container digest-gepinnt, `--network none`; eine Nachbildung im Workflow prüfte nicht das, was lokal läuft. `make -C` gegengeprüft: `CURDIR` zeigt auf `lab/example`, der Mount des Prüfers stimmt. Die Entscheidung, die offen war, ist damit getroffen — `main` färbt sich künftig rot, sobald das Beispiel driftet |
