...
| Faden | Trigger | Stand |
|---|---|---|
| **`lab/example` hat keine Durchsetzungsschicht** — `make -C lab/example verify` steht in keinem Workflow: weder `check_closure_notes.py` noch die vier d-check-Module des Beispiels (`matrix`, `targets`, `planning`, `ids`) laufen in CI. `checks.yml` trägt im eigenen Kopf den Satz, den das verletzt — *„Ein Gate, das nur lokal läuft, ist ein Vorschlag"* (Modul 13) | **eingetreten** — der Widerspruch steht seit Welle 72 im Repo und ist mit der Closure-Verdrahtung in Welle 73 breiter geworden | offen seit Welle 73, aufgefallen beim Review der Closure-Verdrahtung. Vorbedingung geprüft: Die Gates sind lokal grün und hermetisch (Docker, digest-gepinnt, `--network none`), ein Job wäre also nicht mehr als ein `make`-Aufruf. Offen ist die Entscheidung, nicht die Machbarkeit: Ein neuer Job färbt `main` rot, sobald das Beispiel driftet — das ist der Zweck, aber es ist eine Setzung über den Merge-Pfad |
...
*Keiner offen.* Der letzte war `v5.4.0` (erreicht 2026-08-15) — Beleg in der
...
|---|---|---|---|
| 2026-08-15 | Meilenstein **v5.4.0** | erreicht und eingetragen | Welle 73 („Zwei Kopien, zwei Antworten"), Inhalt im [`CHANGELOG.md`](../CHANGELOG.md). Belege: Tag `v5.4.0`, Workflow `templates-release` grün (Lauf 31892626291), `checks` auf `main` grün (Spitze `a2e4854`), Asset `lab-regelwerk.zip` (188.758 Bytes, **51 Dateien**), `releases/latest` zeigt auf `v5.4.0`. Stichprobe im **veröffentlichten** Bundle, nicht im lokalen Build: `regelwerk/README.md` steht auf Kurs-Welle 73; die Adress-Regel für Adaptionen steht in `grundlagen-harness-dateien.md`; `templates/harness/conventions.template.md` trägt drei `<a id="mr-<NNN>">`; Ausgang 5 des Freshness-Audits nennt beide Zweige; `modul-11-verification.md` trägt „Ein selbstgebautes Gate ist auf Zeit gebaut"; `grundlagen-traceability.md` trägt den Zeiger statt des nackten Ankers; und in `closure-note-reviewer.template.md` kommt `check_closure_notes` **null** Mal vor, in `modul-13` die falsche Zahl „fünf Schritte" ebenfalls null Mal. Der Release-Rewrite gegengeprüft: Gegenüber dem Repo-Stand unterscheidet sich genau **eine** Zeile je Spiegel — der Quell-Link, auf den Tag gepinnt. **MINOR gewählt:** Drei Regeln kommen hinzu (Adress-Regel für Adaptionen, beide Zweige in Ausgang 5, achter Schritt in Modul 11), dazu zwei Korrekturen; nichts entfällt, kein Asset entfernt, kein Layout gebrochen. **Methoden-Notiz:** Die erste Stichprobe meldete Ausgang 5 als fehlend — ein Falsch-Negativ des eigenen `grep`, weil der Satz über einen Zeilenumbruch läuft. Eine Stichprobe, die zeilenweise sucht, prüft nicht, was sie zu prüfen behauptet |
| 2026-08-10 | Faden **`check_closure_notes.py` retiren** | geschlossen — **nicht** durch Retiren, sondern durch eine Rollen-Entscheidung ([ADR-0019](../lab/example/docs/plan/adr/0019-closure-sensor-und-skript-rolle.md)) | Der Doku-Prüfer erreichte mit v0.56.0 volle Kongruenz: gleiche Kandidaten-Menge (`done/*.md`), gleiche vier Bedingungen, gleiche Schwelle — belegt mit 12 Break-Tests über alle drei Dateiarten des Ruheorts, je beide Sensoren am selben Fund, Bestand beidseitig still. Technisch war das Skript damit ablösbar. **Der Fußabdruck sagte etwas anderes:** [Modul 11](../kurs/de/04-qualitaet/modul-11-verification.md) ist das Worked Example *„Fitness Function ohne Standard-Tool"* und verlinkt das Skript per Pfad als Vergleichsgegenstand. Es zu löschen hätte **den Kurs** falsch gemacht, nicht das Beispiel richtig. Entschieden: Deckung trägt `planning.closure`, das Skript trägt die Lehre; beide Rollen sind an den drei Stellen ausgeschrieben, die es als Gate führten. **Zwei eigene Fehler dabei korrigiert:** Die Schwelle stand auf der Werkzeug-Vorbelegung 4 statt auf den zwei Sätzen der ADR — ein Gate schärfer als seine Entscheidung; und der Kandidatenfilter war auf `slice-*.md` verengt, obwohl ADR-0011 `welle-1-mvp.md` ausdrücklich mitzählt |
