...
| **Team-Sim misst die Closure-Seite und die Vorvergabe nicht.** Die 16 Verdikte decken Anspruch (s01, s06), Vergabe (s02, s05), Register (s03, s07) und die Wellen-Eröffnung (s04) — nicht: eine Welle schließen (`git mv` nach `done/`, Register-Zeile), während ein offener PR noch einen Slice dieser Welle beansprucht (`planning`/`waves` müssten `wave-results-missing`/`planning-drift` sehen); `slice-002` im Wellen-Plan vorvergeben ohne Datei, von bob anders angelegt ([TB-010](team.md#tb-010): „lokal ableitbar" gilt nicht — Erwartung **still**, und das wäre der Befund); zwei Schreiber setzen sich im selben PR-Paar als `Verantwortlich:` desselben Slice (TA-1: laut oder still?); `doc-immutable` auf der Range zweier Clones, die dieselbe Accepted-ADR ändern. **Handlung:** vier Szenarien s08–s11 mit vorab notierter Erwartung, je mit der Kursstelle, die sie trägt | eine Kursaussage zu Wellen-Closure, Vorvergabe oder Rolleninhaber unter Nebenläufigkeit wird geschrieben oder geändert — oder ein Adopter meldet einen dieser Fälle | offen seit Welle 87; Runner trägt seit Welle 87 Auswahl, Ergebnisdatei und `KAPUTT`-Wächter, die Szenarien fehlen |
