...
| **`spezifikation.template.md` trägt `Letzte Änderung` neben seiner Historie — ohne Quell-Satz.** Modul 3 nennt das Frische-Datum nur für die Sicht (*„Keine Historie, nur ein Frische-Datum"*; Vertrag und Technik führen eine Historie, weil ihr Änderungs-Prozess einen Urheber hat); das Technik-Template trägt trotzdem beides — Kopfzeile `Status: Aktiv. Letzte Änderung:` und `## 7. Historie` —, das Beispiel pflegt die Kopfzeile (08-08 bei letztem Commit 08-09). Ein Template-Feld ohne Quell-Verankerung. **Handlung:** entweder Modul 3 §Die Spezifikation sagt, dass die Technik zusätzlich ein Frische-Datum trägt, oder das Feld fällt und die letzte Historie-Zeile trägt das Datum | die nächste Änderung an `spezifikation.template.md` oder an Modul 3 §Die Spezifikation | offen seit Welle 84 |
