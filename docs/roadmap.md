...
| **Mechanische Wächter gegen Doku-Drift** — zwei Prüfungen, die `make check` heute nicht hat: (a) Existenz-Prüfung für Prosa-Pointer der Form „steht in Modul N" / „§Abschnitt", (b) Mengengleichheit geänderter Aufzählungen über Quelle · Regelwerk-Spiegel · Lösung · Rubrik | **eine weitere Welle produziert einen Pointer ins Leere oder eine Aufzählung, die in einer der vier Fassungen abweicht** — dann ist es kein Einzelfall mehr | offen seit Welle 50. Fünf Review-Runden fanden dieselben drei Fehlerklassen: Echo nicht mitgezogen · Pointer ohne Ziel · Ausgang nicht ausführbar. Mindestens vier Befunde wären maschinell gefallen; `d-check` prüft Links, aber keine Prosa-Behauptungen über Zielinhalte |
| **Spec-Strata-Adaptionsrichtung widersprüchlich** — `harness/README.template.md` erklärt 3 Straten zum Default und 2 zur Adaption, `conventions.template.md` und `lab/example/harness/conventions.md` (`MR-001`) genau umgekehrt; beides reist im selben Bundle | **ein Adopter meldet, dass er nicht weiß, ob er für 2 oder für 3 Straten eine `MR` braucht** | offen seit Welle 50; vorbestehend, nicht von Welle 50 verursacht. Die Richtung ist eine Setzung, keine Redaktion — vermutlich Baseline = Regelwerk (2 Straten), Templates zeigen die adaptierte Form |
...
| **Formcheck des Adaptions-Durchgangs ist Prosa statt Gate** — „lösen Geltungsbereich und Begründungs-Verweise noch auf?" ([Modul 2 §Freshness-Audit](../kurs/de/01-spec-und-architektur/modul-02-harness-bootstrap.md)) leistet das Doku-Gate bereits, sofern `.harness/baseline/<tag>/` im Prüfumfang liegt | **ein Adopter nimmt die vendored Baseline in seinen `d-check`-Prüfumfang auf** — dann kann der Prosa-Schritt auf einen Halbsatz schrumpfen | offen seit Welle 50 |
