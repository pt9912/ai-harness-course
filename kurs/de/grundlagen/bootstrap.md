...
Sub-Areas ein Slice einen Begründungsblock schreibt ([Modul 5 §8](../02-planung/modul-05-planning-harness.md#worked-mini-example-bootstrap-modus-pro-sub-area-für-einen-slice-begründen)) und unter welche
Sub-Area eine Beobachtung ins Register geht ([Modul 6 §Das Beobachtungs-Register](../02-planung/modul-06-roadmap.md#das-beobachtungs-register)). Zwei Wege führen dorthin,
...
| **Cross-Reference-Trigger** | Verlinkung zwischen Dokumenten, normativ **nur volatil→stabil** ([§Referenz-Richtung](referenz-richtung.md#referenz-richtung-sdp-wer-darf-wen-referenzieren)) | Neue ADR *deklariert aufwärts, was sie schärft* (ADR → Spec-§) und referenziert die Anforderung; der Acceptance-Trigger zieht die Spec nach. Ein Spec→ADR-Rückzeiger im bindenden Text existiert nicht (auch nicht als Quellen-Spalte) — Provenance nur in der Historie-Tabelle (Regel 5); `check-references` erzwingt das über alle Straten |
...
- **[§harness/conventions.md als Konventionsspeicher](harness-dateien.md#harnessconventionsmd-als-konventionsspeicher)**: Adaptions-Block
