...
| Reconciliation-Backlog | Sicht, kein Artefakt: die offenen `CO-DS-*` plus die Reconciliation-Slices in `open/`. *„Steht“* heißt — jede Diskrepanz der Inventur ist in genau eine der drei Klassen einsortiert (`CO-DS-*` · Reconciliation-Slice · retro-ADR), nicht „ist leer“. Closure-Kriterium des Brownfield-Bootstraps, siehe [§Harness-Bootstrap-Ende](bootstrap.md#harness-bootstrap-ende-vs-workflow-beginn). |
...
| Stratum | Rollen-Klasse eines Spec-Dokuments — *Vertrag* (Decke) · *Technik* · *Sicht* —, bestimmt über normativen Gehalt und Änderungs-Prozess, nicht über den Dateinamen. Rang: Vertrag › Technik › Sicht; alle drei sind obligatorisch, eine Abweichung wird als `MR-<NNN>` deklariert. Siehe [§Spec-Straten](referenz-richtung.md#spec-straten-mehr-als-ein-spec-dokument). |
