...
die Wahl ist Repo-spezifisch. Genau diese Wahlfreiheit ist die
Erschaffens-Leistung dieses Moduls.
...
| Drei Fundstücke: DoD-Verletzung oder Review-Finding? | alles "irgendwie Findings", ohne Artefakt-Bezug | A = DoD-Verletzung (DoD-Punkt "Gates grün" nicht erfüllt — Verifier prüft gegen DoD) · B = Review-Finding (Maintainability-Beobachtung am Diff, kein DoD-/Plan-Bezug — Reviewer prüft gegen Plan/ADR/Konventionen) · C = DoD-/Plan-Verletzung (Plan-gegen-Code-Differenz — genau die Frage "wurde umgesetzt, was geplant war?", die nur die Verifikation stellt). | + Abgrenzungs-Kriterium explizit: nicht die *Schwere* entscheidet, sondern das *Prüf-Artefakt* (DoD/Spec/Plan → Verifier; Plan/ADR/Diff → Reviewer). Pointe zu C: ein Reviewer mit veraltetem Plan übersieht die Differenz strukturell — deshalb ist sie eine eigene Klasse, die *nur* die Verifikation fängt (siehe §Vorgriff, DoD-Verletzung). |
| Pre-completion-Bericht gegen Plan/DoD bewerten + Lücke benennen? | "Klingt ok, läuft ja." | Bericht gegen Plan/DoD geprüft, nicht gegen sich selbst: "Tests grün" deckt nicht den verlangten Negativtest, "läuft" nicht den fehlenden Closure-Note. Mindestens eine konkrete DoD-Lücke benannt (z. B. Negativtest fehlt). | + Selbstabsolution-Floskeln ("minimal", "läuft") als Warnsignal erkannt: der Bericht prüft sich selbst statt gegen die DoD; beide Lücken plus Anschluss benannt — der Verifier kategorisiert sie als HIGH/MEDIUM-Findings, nicht der Bericht. |
